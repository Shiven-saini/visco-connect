...
void CameraApiService::onDeleteCameraFinished()
{
    ...
    if (statusCode == 200 || statusCode == 204) {
        ...
        QString error = QString("Server returned status code: %1").arg(statusCode);
        LOG_ERROR(QString("Failed to delete camera on server: %1 - %2").arg(cameraId, error), "CameraApiService");
        emit cameraDeleted(cameraId, false, error);
        showApiError("delete camera", error);
    }
}
